...
# has salt lake city but no nashville
all2018 <- read.csv(("all2018.csv"))
...
all6$hometown <- as.factor(all6$hometown)
...
all <- bind_rows(all6, all7,all8)

```
```{r save this file for future use}
write.csv(all, "all_master.csv")
```
