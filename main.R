...
all$scientific_name <- str_replace(all$scientific_name,"Columba livia domestica", "Columba livia")
all$scientific_name <- as.factor(all$scientific_name)

# data subsets for later use
plants <- all %>% filter(taxon_class_name %in% c("Magnoliopsida", "Liliopsida", "Polypodiopsida", "Pinopsida", "Agaricomycetes", "Lecanoromycetes"))
animals <- all %>% filter(taxon_class_name %in% c("Arachnida", "Aves", "Gastropoda", "Insecta", "Amphibia", "Reptilia", "Mammalia"))
dicots <- all %>% filter(taxon_class_name == "Magnoliopsida") %>% mutate (taxon="dicots")
monocots <- all %>% filter(taxon_class_name == "Liliopsida") %>% mutate (taxon="monocots")
ferns <- all %>% filter(taxon_class_name == "Polypodiopsida")%>% mutate (taxon="ferns")
conifers <- all %>% filter(taxon_class_name == "Pinopsida") %>% mutate (taxon="conifers")
birds <- all %>% filter(taxon_class_name == "Aves") %>% mutate (taxon="birds")
insects <- all %>% filter(taxon_class_name == "Insecta") %>% mutate (taxon="insects")
arachnids <- all %>% filter(taxon_class_name == "Arachnida") %>% mutate (taxon="arachnids")
reptiles <- all %>% filter(taxon_class_name == "Reptilia") %>% mutate (taxon="reptiles")
amphibians <- all %>% filter(taxon_class_name == "Amphibia") %>% mutate (taxon="amphibians")
mammals <- all %>% filter(taxon_class_name == "Mammalia") %>% mutate (taxon="mammals")
gastropods <- all %>% filter(taxon_class_name == "Gastropoda") %>% mutate (taxon="gastropods")
...
tab_all <- adonis.table.hometown(all) %>% mutate (taxon = "all")
...
tab_all <- adonis.table.lc(all) %>% mutate (taxon = "all")
